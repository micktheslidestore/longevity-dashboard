...
export function ChatWidget() {
  ...
  const isVisible = !isCommandPage && (isCoach || isClient)
  ...
  // Now safe to return null — all hooks are above this line
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontFamily: "var(--sans)", fontSize: 10, color: accentColor, marginTop: 1 }}>
          ...
          <button
            ...
            style={{ background: "transparent", border: "none", cursor: "pointer", padding: 4, color: "var(--ink-3)", display: "flex", alignItems: "center", justifyContent: "center" }}
          >
        ...
        {/* Quick chips */}
        <div style={{ padding: "8px 12px", borderBottom: "1px solid var(--hair)", display: "flex", gap: 6, flexWrap: "wrap", flexShrink: 0 }}>
          {CHIPS.map(chip => (
            <button
              key={chip}
              onClick={() => send(chip)}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "12px 14px", display: "flex", flexDirection: "column", gap: 10 }}>
          {messages.map((m, i) => (
            <div key={i} style={{ display: "flex", flexDirection: m.role === "user" ? "row-reverse" : "row", gap: 8, alignItems: "flex-start" }}>
              ...
              <div style={{
                maxWidth: "78%",
                background: m.role === "agent" ? "var(--panel-2)" : `color-mix(in srgb, ${accentColor} 10%, var(--panel-2))`,
                border: `1px solid ${m.role === "agent" ? "var(--hair)" : `color-mix(in srgb, ${accentColor} 25%, transparent)`}`,
                padding: "8px 11px",
                borderRadius: 8,
              }}>
                <div style={{ fontFamily: "var(--sans)", fontSize: 10, color: m.role === "agent" ? accentColor : "var(--ink-3)", marginBottom: 4 }}>
                  {m.role === "agent" ? "Agent" : userName}
                </div>
                <div style={{ fontSize: 11.5, color: "var(--ink-2)", lineHeight: 1.55, whiteSpace: "pre-line" }}>
                  {m.text}
                </div>
              </div>
            </div>
          ...
          {loading && (
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <div style={{ width: 22, height: 22, borderRadius: "50%", background: "var(--panel-2)", border: "1px solid var(--hair-strong)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--sans)", fontSize: 9, color: accentColor }}>A</div>
              <div style={{ fontFamily: "var(--sans)", fontSize: 11, color: "var(--ink-4)" }}>Analysing…</div>
            </div>
          )}
          <div ref={bottomRef} />
        </div>
        ...
        {/* Input */}
        <div style={{ padding: "10px 12px", borderTop: "1px solid var(--hair)", display: "flex", gap: 8, flexShrink: 0 }}>
          <input
            ref={inputRef}
            value={input}
            ...
            style={{
              flex: 1, background: "var(--panel-2)", border: "1px solid var(--hair-strong)",
              padding: "7px 10px", fontFamily: "var(--sans)", fontSize: 12,
              color: "var(--ink)", outline: "none", borderRadius: 6,
            }}
          />
          <button
            onClick={() => send(input)}
            style={{
              ...
              border: "none", background: accentColor, color: "var(--bg)",
      ...
      >
        {open ? (
          ...
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
            <path d="M20 2H4C2.9 2 2 2.9 2 4V16C2 17.1 2.9 18 4 18H7L12 23L17 18H20C21.1 18 22 17.1 22 16V4C22 2.9 21.1 2 20 2Z" fill="white" />
          </svg>
        )}
      </button>
    </>
